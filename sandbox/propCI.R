...
#' @param interval number number of intervals to make (limit and default are 50)
...
watch_statisticians <- function(n, p, interval_number = 50, level = 0.95) {
  ...
  df <- make_intervals(
    n = n,
    p_hat = rbinom(interval_number, size = n, prob = p) / n,
    level = level,
    p = p
  )
  ggplot2::ggplot(df, ggplot2::aes(x = int, y = upper)) +
    ggplot2::geom_segment(ggplot2::aes(
      x = int, xend = int, 
      y = lower, yend = upper, 
      color = lemon)) +
    ggplot2::geom_point(ggplot2::aes(x = int, 
                            y = p_hats, 
                            color = lemon), 
                        size = 1) +
...
watch_statisticians_slow <- function(n, p, 
  ...
  while(count < interval_number) {
    ...
    plot_so_far <-
      ggplot2::ggplot(df, aes(x = int, y = upper)) +
      ggplot2::geom_segment(ggplot2::aes(
        x = int, xend = int, 
        y = lower, yend = upper, 
        color = lemon)) +
