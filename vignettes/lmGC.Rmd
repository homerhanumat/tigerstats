...
We predict that a 3000 pound car (from the time of the Motor Trend study that produced this data) would have a fuel efficiency of 21.25 mpg, give or take about 3.095 mpg or so.
...
A n ice way to check whether your fit is appropriate to the data is to use the `check` argument for `lmGC()`:
...
We get the linear fit, but there is also a "loess" curve, a smooth curve that does not make much assumption about what kind of process produced the data in question.  Surrounding the loess curve is a 95%-confidence band:  based on the data we are pretty sure that the "real" relationship between **avgtemp** and **volume** is some curve that lies within the band.  The regression line wander a bit outside the band, so a linear fit probably is not quite adequate for this data.  Perhaps we should learn how to fit some other types of curves to the data!
