...
Note that regression line wanders a bit outside the band, so a linear fit probably is not quite adequate--especially for days with higher temperatures, when people may forsake bicyle trials for other warm-weather activities such as swimming.  At very high temperatures outside the range for which we have data, **volume** should be low, so the "real" relationship is probably quite curvilinear.  Perhaps we should learn how to fit something other than a straight line to data!
