...
You can also get a *predcition interval* for the response variable if you use the `level` argument:
...
We get the linear fit, but there is also a "loess" curve, a smooth curve that does not make much assumption about what kind of process produced the data in question.  Surrounding the loess curve is a 95%-confidence band:  based on the data we are pretty sure that the "real" relationship between **avgtemp** and **volume** is some function with a graph that lies within the band.  The regression line wanders a bit outside the band, so a linear fit probably is not quite adequate.  Perhaps we should learn how to fit some other types of curves to the data!
